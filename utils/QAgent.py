...
class Q_agent_base(): 
  def __init__(self,starting_actions=2): #starting action is how many actions are initially present. 
    ...
    self.QM=dynamic_QM()
    for _ in range(starting_actions-1):
      self.QM.add_action()
  ...
  def reward(self,state,action,reward): 
    try: #to ensure that only states that exist and actions that exist are used. 
     ...
     self.QM.table[self.states.index(state)][action]=reward
    except:
      pass
  ...
  def return_lowest_index (self, state): 
    return np.argmin(self.QM.summary()[self.states.index(state)] )

  def return_Q (self,state,action): 
    return self.QM.table[self.states.index(state)][action]
